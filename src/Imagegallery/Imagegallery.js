...
class Imagegallery extends Component{
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.currentPage!== this.state.currentPage || prevState.pageLimit!== this.state.pageLimit ) {
      this.getImages();
    }
  }
  ...
  getImages (){
    ...
    axios.post(APIURL+`/photos/list`,data)
      .then(res => {
        this.setState({
  ...
  deleteImage (album, filename) {
    axios.delete(APIURL + `/photos/` + album.toLowerCase() + '/' + filename , {})
      .then(res => {
  ...
  render() {
    ...
    const pageLimitSelect = (
      <div className="pagelimit">
        Show &nbsp;
        <select id="pageLimit" onChange={this.handleLimitChange.bind(this)} value={this.state.pageLimit} >
      ...
      <Aux >
        ...
        <div className="imagegallery">
          {this.state.images.map((image) =>
            <Imagefigure image={image} key={image.id} deleteImage={this.deleteImage.bind(this)}/>
            )
          }
        </div>
        <ul id="page-numbers">
